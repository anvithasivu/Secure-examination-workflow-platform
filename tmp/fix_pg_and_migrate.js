...
async function migrate() {
    ...
    // FIX SCHEMA
    try {
        await pgClient.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS course_id INT NULL");
        log("Added course_id to users schema in pg.");
    } catch (e) {
        log("Schema fix warning: " + e.message);
    }
    ...
    try {
        // Truncate all tables starting from the dependent ones
                    ...
                    for (const row of batch) {
                        ...
                        try {
                            const params = columns.map(col => {
                                // PostgreSQL requires boolean true/false for tinyint if mapped specifically, but we map as INT
                                return row[col];
                            });
                            await pgClient.query(rowQuery, params);
                            successCount++;
                        } catch(rowErr) {
